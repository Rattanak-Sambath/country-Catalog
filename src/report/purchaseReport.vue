<template>
          ...
          >
            <thead>
              <tr>
                ...
                <th v-if="showMoreHeader('purchaseType')">Purchase Type</th>
            ...
            <tbody>
              ...
              >
                ...
                <td v-if="showMoreHeader('staff')"><b>{{ doc.staffName }}</b></td>
                <td v-if="showMoreHeader('purchaseType')">{{ doc.type }}</td>
  ...
  export default {
    ...
    //   })
      ...
      const supplierNameOpts = ref([])
      const staffNameOpts = ref([])
      const jobNameOpts = ref([])
      const branchName = ref('')  
      const purchaseTypeOpts = ref([
        {
          label: 'Bill',
          value: 'Bill',
        },
    ...
    //   }
      ...
      const onSubmit = async () => {
        ...
        if (valid) {
          ...
          // console.log('doc', doc);
          await api.get('/purchase/purchaseReport', {
            params: doc
          }).then((res)=>{
            if(res){
      ...
      const getSupplierOpts = async () => {
        ...
        await api
                .get('/supplier/getSupplierReport' , selector)
                .then((res) => {   
                    if (res) {
                        supplierOpts.value = res.data
                    }
                    })
                .catch((err) => {
                console.log(err)
      ...
      const handleSupplierChange = (newVal) => {
        let suppplierName = []
        if (newVal) {
            suppplierName = supplierOpts.value
            .filter((it) => newVal.includes(it._id))
            .map((it) => it.name)
            supplierNameOpts.value = suppplierName
        } else {
