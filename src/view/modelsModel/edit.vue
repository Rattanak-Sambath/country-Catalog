  ...
  <div>
    <q-card class="flex justify-space-between">
      ...
      <q-toolbar>
        <q-toolbar-title class="text-h6 text-bold"
          ><q-icon name="add"></q-icon> Edit Models</q-toolbar-title
        >
        <q-space />
        <q-btn
          icon="west"
    ...
    <q-card class="q-my-md">
      <q-card-section class="text-grey-15">
        Fill the form below to crate new Models
      </q-card-section>
      <ValidateForm
        ref="formRef"
...
const onUpdate = async () => {
  ...
  if (valid) {
    ...
    const res = await api.put(
      `model/updateModel/` + $route.params.id,
      form.value
    )
    if (res) {
      toast.success({ message: 'Update model successfully' })
      cancel()
    } else {
...
const findDatabyId = async () => {
  let id = showId.value
  // console.log('find', id);
  let res = await api.get(`/model/getModelbyId/` + $route.params.id)
  // console.log(res)
  if (res) {
