    ...
    <q-card class="flex justify-space-between">
      <q-toolbar>
        <q-toolbar-title class="text-h6 text-bold"
          ><q-icon name="add"></q-icon> Create Supplier</q-toolbar-title
        >
        <q-space />
        <q-btn
          icon="west"
...
const onSubmit = async () => {
  ...
  if (valid) {
    ...
    await api.post('supplier/createSupplier', form.value).then((res)=>{
     if(res){
...
const countSupplier = async ( )=>{
  await api.get('supplier/countSupplier').then((res) => {
    if(res){
